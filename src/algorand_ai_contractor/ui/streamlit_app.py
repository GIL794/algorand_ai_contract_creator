...
# Define outputs path (go up 3 levels: ui -> algorand_ai_contractor -> src -> project root)
# Calculate project root: from ui/streamlit_app.py -> algorand_ai_contractor -> src -> project root
...
with st.sidebar:
    ...
    st.warning("⚠️ **TESTNET ONLY** - Generated contracts are for testing purposes. Do NOT use for production/mainnet without security audit.")
    
    st.subheader("🔑 Testnet Address Generator")
    if st.button("Generate Testnet Address", help="Generate a new Algorand testnet address for use in contracts"):
        if deployer:
            ...
            st.session_state['generated_testnet_address'] = test_account  # Keep for backward compatibility
    ...
    available_addresses = []
    if 'generated_testnet_addresses' in st.session_state and st.session_state['generated_testnet_addresses']:
    ...
    if available_addresses:
        with st.expander(f"📋 Generated Testnet Addresses ({len(available_addresses)})", expanded=True):
            for idx, account in enumerate(available_addresses, 1):
                ...
                with col1:
                    if st.button("🌐 Connect Wallet", key=f"sidebar_wallet_{idx}", use_container_width=True):
            ...
            st.info(f"💡 All {len(available_addresses)} address(es) will be automatically included in contract generation!")
...
with tab1:
    ...
    st.warning("⚠️ **TESTNET ONLY**: Contracts generated here are for Algorand TestNet testing. They are NOT production-ready and require security audits before mainnet deployment.")
    ...
    if available_addresses:
        ...
        st.success(f"💡 **{len(available_addresses)} Testnet Address(es) Available**: {addresses_text}")
        st.caption("These addresses will be automatically included in your contract generation. You can reference them in your description or they'll be added automatically.")
    ...
    if (current_contract and 
        isinstance(current_contract, dict) and
        current_contract.get('success', False)):
        ...
        # Testnet warning
        st.warning("⚠️ **TESTNET CONTRACT**: This contract is generated for Algorand TestNet only. Replace any placeholder addresses with actual testnet addresses before deployment.")
        ...
        if available_addresses:
            with st.expander(f"🔑 Available Testnet Addresses ({len(available_addresses)})", expanded=False):
                for idx, account in enumerate(available_addresses, 1):
                    st.markdown(f"**Address {idx}:**")
                    st.code(f"{account['address']}", language='text')
                    st.caption(f"Used in contract generation. Private key and mnemonic available in sidebar.")
                    if idx < len(available_addresses):
                        st.divider()
        ...
        with st.expander("🛠 Deployment Instructions"):
            ...
            st.caption("⚠️ Remember: This is for TESTNET only. Do not deploy to mainnet without security audit.")
        
        # Audit summary
        with st.expander("🔒 Security Audit Summary"):
            st.markdown(contract.get('audit') or "No audit information provided")
            st.warning("⚠️ **IMPORTANT**: This code has NOT been security audited. Always audit smart contracts before deploying to mainnet with real value.")
...
with tab3:
    ...
    st.error("🚨 **TESTNET DEPLOYMENT ONLY** - This will deploy to Algorand TestNet. Never use testnet contracts on mainnet without security audit.")
        ...
        with st.spinner("Compiling..."):
            ...
            if '---' in cleaned_code:
                ...
                for line in lines:
                    ...
                    if any(header in stripped for header in [
                        ...
                        '**Contract Purpose', '**Logic Walkthrough', '**Security'
                    ]):
                        break
            ...
            if compile_result.get('success', False):
                ...
                with st.expander("🔍 View code that failed to compile", expanded=True):
                    ...
                    st.caption("💡 Tip: The error message above shows the problematic line. Check for unmatched quotes, incomplete strings, or syntax errors.")
    ...
    if 'compiled_teal' in st.session_state:
        ...
        with st.expander("🆕 Generate Test Account", expanded=True):
            st.caption("Generate a new Algorand testnet account for deployment. This account will need to be funded via the TestNet faucet.")
            if st.button("Create New Testnet Account"):
                ...
                st.json({
                    ...
                    'faucet_url': test_account['faucet_url']
                })
                st.warning("⚠️ **IMPORTANT**: Save your private key and mnemonic securely. This is for TESTNET only.")
        ...
        if 'deployment_testnet_account' in st.session_state:
            ...
            st.info(f"📋 **Deployment Account**: `{account['address']}` - Make sure it's funded before deploying!")
            
            # Wallet funding option for deployment account
            col1, col2 = st.columns(2)
            with col1:
                if st.button("🌐 Connect Web3 Wallet to Fund", key="deploy_wallet_connect", use_container_width=True):
                    st.session_state['show_wallet_modal'] = True
                    st.session_state['wallet_funding_address'] = account['address']
                    st.rerun()
            with col2:
...
with tab4:
    ...
    if not st.session_state.generation_history:
        ...
        for idx, entry in enumerate(reversed(st.session_state.generation_history)):
            ...
            with st.expander(f"Contract #{len(st.session_state.generation_history) - idx} - {timestamp}"):
...
if st.session_state.get('show_wallet_modal', False):
    ...
    for idx, wallet in enumerate(wallets):
        with cols[idx % 2]:
            with st.container():
                ...
                with col_btn1:
                    if st.button("Connect", key=f"connect_{wallet['name']}", use_container_width=True):
                        st.info(f"💡 **Note**: Connect {wallet['name']} and use it to send testnet ALGO to: `{funding_address}`")
                        st.session_state[f'wallet_connected_{wallet["name"]}'] = True
                        # Open wallet website in new tab
                        st.markdown(f'<a href="{wallet["connect_url"]}" target="_blank">Open {wallet["name"]}</a>', unsafe_allow_html=True)
                with col_btn2:
    ...
    **To fund this account manually:**
    ...
    # Close modal button
    if st.button("✖ Close Wallet Selection", key="close_wallet_modal", use_container_width=True, type="secondary"):
