    ...
    def _get_client(self, provider: str) -> OpenAI:
        ...
        if provider == "perplexity":
            if not PERPLEXITY_API_KEY or PERPLEXITY_API_KEY == "pplx-your-key-here":
                raise ValueError(
                    "PERPLEXITY_API_KEY not set. Please configure it in your .env file."
                )
            ...
            try:
                ...
                response = client.chat.completions.create(
                    ...
                    messages=[
                        ...
                        {
                            "role": "user",
                            "content": self._build_user_prompt(description, last_error),
                        },
                    ],
                ...
                if validation_result.get("valid", False):
                    self._log_generation(description, parsed, attempt + 1, provider, selected_model)
                    return {
                        "success": True,
                        "code": code,
                ...
                if (
                    "Syntax Error" in last_error
                    or "syntax error" in last_error.lower()
                    or "invalid syntax" in last_error.lower()
                ):
...
"""
    ...
    def _parse_ai_response(self, raw_output: str) -> Dict[str, str]:
        ...
        if code_start != -1:
            ...
            if code_end != -1:
                ...
                for i, line in enumerate(code_lines):
                    ...
                    if (
                        re.match(
                            r"^(from pyteal|import pyteal|from\s+pyteal|def\s+\w+|class\s+\w+)",
                            stripped,
                            re.IGNORECASE,
                        )
                ...
                if python_start_idx >= 0:
                    ...
                    for i in range(python_start_idx, len(code_lines)):
                        ...
                        if (stripped.startswith("##") or stripped.startswith("###")) and not any(
                            c in line for c in ['"', "'", "#"]
                        ):
        ...
        if code:
            ...
            for line in lines:
                ...
                if lines_collected < 5 or (
                    stripped == "---" and not any(c in line for c in ['"', "'", "=", "#"])
                ):
                    if stripped == "---" or (
                        stripped.startswith("---")
                        and len(stripped) >= 3
                ...
                if not found_python_start:
                    ...
                    if (
                        stripped.startswith(("from ", "import ", "def ", "class ", "#"))
                        or re.match(r"^[A-Z_][A-Z0-9_]*\s*=", stripped)  # Variable assignment
                        or re.match(r"^[a-z_][a-z0-9_]*\s*\(", stripped)
                    ):  # Function call
                ...
                if (
                    found_python_start
                    or stripped.startswith(("from ", "import ", "def ", "class ", "#"))
                    or not stripped
                ):
        ...
        if code:
            ...
            for line in code.split("\n"):
                ...
                if stripped and (
                    stripped.startswith("```") or stripped == "```python" or stripped == "```py"
                ):
                    continue
        ...
        if explanation_text:
            ...
            for pattern in purpose_patterns:
                ...
                if purpose_match:
                    ...
                    purpose_text = re.sub(
                        r"^#+\s*[📝]*\s*(?:Contract Purpose|Purpose Summary|Purpose)[:\s]*\n?",
                        "",
                        purpose_text,
                    ...
                    purpose_text = re.sub(
                        r"^---+\s*$", "", purpose_text, flags=re.MULTILINE
                    ).strip()
                    if purpose_text:
            ...
            for pattern in security_patterns:
                ...
                if security_match:
                    ...
                    security_text = re.sub(
                        r"^#+\s*[🔐]*\s*Security (Considerations|Audit|Summary)[:\s]*\n?",
                        "",
                        security_text,
                        flags=re.IGNORECASE | re.MULTILINE,
                    )
                    ...
                    security_text = re.sub(
                        r"^---+\s*$", "", security_text, flags=re.MULTILINE
                    ).strip()
                    if security_text:
            ...
            for pattern in deployment_patterns:
                deployment_match = re.search(
                    pattern, explanation_text, re.IGNORECASE | re.MULTILINE
                )
                if deployment_match:
                    deployment_text = deployment_match.group(0).strip()
                    deployment_text = re.sub(
                        r"^#+\s*[🚀]*\s*Deployment (Parameters|Instructions|Needed)[:\s]*\n?",
                        "",
                        deployment_text,
                        flags=re.IGNORECASE | re.MULTILINE,
                    )
                    ...
                    deployment_text = re.sub(
                        r"^---+\s*$", "", deployment_text, flags=re.MULTILINE
                    ).strip()
                    if deployment_text:
            ...
            if not deployment and explanation_text:
                ...
                deployment_fallback = re.search(
                    r"[Dd]eployment[:\s]*[\s\S]*?(?=\n---|\n###|\n##|Security|References|$)",
                    explanation_text,
        ...
        explanation = (
            re.sub(r"^---+\s*$", "", explanation, flags=re.MULTILINE).strip() if explanation else ""
        )
        deployment = (
            re.sub(r"^---+\s*$", "", deployment, flags=re.MULTILINE).strip() if deployment else ""
        )
        audit = re.sub(r"^---+\s*$", "", audit, flags=re.MULTILINE).strip() if audit else ""

        # Log what we extracted for debugging
        logging.debug(
            f"Extracted code length: {len(code)}, explanation length: {len(explanation)}, "
    ...
    def _validate_pyteal_syntax(self, code: str) -> Dict[str, str]:
        ...
        if not has_clear:
            logging.warning(
                "Generated code missing clear_program() - will use default clear program"
            )
        ...
        if "from pyteal import" not in code and "import pyteal" not in code:
            return {
                "valid": False,
                "error": "Missing PyTeal imports. Code must import from pyteal.",
            }
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if any(
                stripped.startswith(keyword) for keyword in ["if ", "elif ", "while ", "assert "]
            ):
            ...
            for pattern in dangerous_patterns:
                if pattern in stripped:
                    return {
                        "valid": False,
                        "error": f"Dangerous pattern detected at line {line_num}: {pattern}. Security violation.",
                    }
    ...
    def _log_generation(
        self, description: str, parsed: Dict[str, str], attempt: int, provider: str, model: str
    ) -> None:
