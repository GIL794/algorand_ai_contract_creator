...
class AlgorandDeployer:
    ...
    def _verify_connection(self):
        ...
        try:
            ...
            logging.info(
                f"Connected to Algorand TestNet - Round: {status.get('last-round', 'unknown')}"
            )
        except Exception as e:
    ...
    def compile_pyteal_to_teal(self, pyteal_code: str, mode=None) -> Dict:
        ...
        try:
            ...
            if not (
                cleaned_code.startswith("from ")
                or cleaned_code.startswith("import ")
                or cleaned_code.startswith("def ")
                or cleaned_code.startswith("class ")
            ):
                return {
                    "success": False,
            ...
            try:
                ...
                if "unterminated string literal" in str(syntax_err).lower() or "EOL" in str(
                    syntax_err
                ):
                    return {
                        "success": False,
            ...
            from pyteal import Le  # Note: Ge (not Gte), Le (not Lte)
            from pyteal import (Add, Addr, And, App, Approve, Assert, Balance,
                                Btoi, Bytes, Concat, Cond, Div, Eq, Ge, Global,
                                Gt, If, InnerTxnBuilder, Int, Itob, Len, Lt,
                                Minus, Mod, Mode, Mul, Neq, Not, OnComplete,
                                Or, Reject, Return, Seq, Substring, Txn,
                                TxnField, TxnType, compileTeal)
            ...
            return {
                "success": False,
                "error": f"Name Error: {e}. Make sure all PyTeal functions are properly imported.",
            }
...
def create_simple_clear_program() -> str:
    ...
    try:
        ...
        raise ImportError(
            f"Failed to import PyTeal: {str(e)}. Please ensure pyteal is installed and compatible with Python 3.13."
        )
